...
const NFT_TIERS = [
  ...
  async function handleWaitlist(e) {
    ...
    const { error } = await supabase.from('waitlist').insert([{ email, referral_code: refParam || null }]);
    if (!error) {
      ...
      if (refParam) {
        await fetch("/api/referrals/track", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ referral_code: refParam, referred_email: email })
        });
      }
    ...
    <main className="min-h-screen bg-gradient-to-br from-[#0B0C10] via-[#1F2235] to-[#232946] text-white flex flex-col items-center px-4 py-10">
      ...
      <section className="max-w-3xl w-full text-center mb-12">
        <div className="flex justify-center mb-6">
          ...
          <img src="/logo.svg" alt="ReceiptX Logo" width={240} height={100} style={{maxWidth:'320px', height:'auto'}} className="drop-shadow-xl" />
        </div>
      ...
      <section className="w-full max-w-lg mb-14">
        ...
        {waitlistSuccess && (
          <div className="mt-4 text-green-300 text-center">
            Thank you! You’re on the list for early access and updates.<br />
            {walletAddress && (
              <div className="mt-2">
                <span className="block text-cyan-200">Your Supra wallet address:</span>
                <span className="block text-cyan-400 font-mono break-all">{walletAddress}</span>
                <span className="block text-xs text-slate-400 mt-1">Your Early Supporter NFT will be airdropped here at launch.</span>
              </div>
            )}
            {referralLink && (
              <div className="mt-4">
                <span className="block text-cyan-200">Your referral link:</span>
                <span className="block text-cyan-400 font-mono break-all">{referralLink}</span>
                <span className="block text-xs text-slate-400 mt-1">Share this link—earn 5-10 AIA for every friend who joins and uploads a receipt!</span>
              </div>
            )}
          </div>
        )}
        {refParam && !waitlistSuccess && (
          <div className="mt-2 text-cyan-300 text-center text-xs">Referral code detected: <span className="font-mono">{refParam}</span></div>
        )}
      </section>
